"""Tests for AdaPNM — AdaPNM (positive-negative momentum) on kaon's backend.

The numpy reference mirrors AdaPNM's **non-factored (1-D) fp32 path**, which is the
...
def _ref_adapnm_1d(
    ...
    *,
    ...
    Reproduces the kozistr AdaPNM math AdaPNM's 1-D path mirrors:
...
def test_matches_numpy_reference_1d(ams_bound, beta0, weight_decay):
    """AdaPNM's 1-D fp32 path matches the numpy AdaPNM reference (cautious off)."""
...
def test_momentum_dtype_variants_construct_and_step(momentum_dtype):
    ...
    opt = AdaPNM(params, lr=1e-3, momentum_dtype=momentum_dtype)
    for _ in range(3):
        for p in params:
            p.grad = torch.randn_like(p)
        opt.step()
...
def test_two_momenta_present():
    """AdaPNM carries TWO momentum buffers (the PNM signature)."""
    p = torch.nn.Parameter(torch.randn(8, 4))
    opt = AdaPNM([p], lr=1e-3, momentum_dtype="float32")
    p.grad = torch.randn_like(p)
    opt.step()
    st = opt.state[p]
...
def test_int8_is_one_byte_per_param_per_momentum():
    p = torch.nn.Parameter(torch.randn(8, 4))
    opt = AdaPNM([p], lr=1e-3, momentum_dtype="int8")
    p.grad = torch.randn_like(p)
    opt.step()
    st = opt.state[p]
...
def test_4bit_is_half_byte_per_param_per_momentum():
    ...
    opt = AdaPNM([p], lr=1e-3, momentum_dtype="4bit", momentum_4bit_block=16)
    p.grad = torch.randn_like(p)
    opt.step()
    st = opt.state[p]
...
def test_foreach_matches_per_param(cfg):
    ...
    torch.manual_seed(1)
    pb = mk()
    oa = AdaPNM(pa, lr=1e-3, foreach=True, bf16_method="none", **cfg)
    ob = AdaPNM(pb, lr=1e-3, foreach=False, bf16_method="none", **cfg)
    torch.manual_seed(7)
    for _ in range(6):
...
def test_foreach_chunking_is_exact():
    ...
    torch.manual_seed(2)
    pb = mk()
    oa = AdaPNM(pa, lr=1e-3, momentum_dtype="int8", weight_decay=0.02, foreach=True, foreach_stack_budget=120)
    ob = AdaPNM(pb, lr=1e-3, momentum_dtype="int8", weight_decay=0.02, foreach=False)
    torch.manual_seed(3)
    for _ in range(5):
        gs = [torch.randn_like(p) for p in pa]
        for p, g in zip(pa, gs, strict=True):
...
def test_overfits_regression(toy_mlp, random_batch):
    """AdaPNM should drive a tiny MLP's training loss down on a fixed batch."""
    x, y = random_batch
    opt = AdaPNM(toy_mlp.parameters(), lr=3e-3)
    losses = []
    for _ in range(80):
        opt.zero_grad()
...
def test_bf16_weights_train_no_nan():
    ...
    opt = AdaPNM([p], lr=1e-3, bf16_method="stochastic_rounding")
    for _ in range(5):
        p.grad = torch.randn_like(p)
        opt.step()
    assert torch.isfinite(p).all()
...
def test_kahan_runs():
    ...
    opt = AdaPNM([p], lr=1e-3, bf16_method="kahan")
    for _ in range(3):
        p.grad = torch.randn_like(p)
        opt.step()
...
def test_checkpoint_roundtrip_preserves_momentum_dtype(momentum_dtype):
    ...
    torch's default load_state_dict upcasts state to the param dtype (fp32); AdaPNM
    ...
    opt_a = AdaPNM(params_a, lr=1e-3, momentum_dtype=momentum_dtype, weight_decay=0.01)
    for _ in range(3):
        for p in params_a:
            p.grad = torch.randn_like(p)
        opt_a.step()
    ...
    opt_b = AdaPNM(params_b, lr=1e-3, momentum_dtype=momentum_dtype, weight_decay=0.01)
...
def test_conv_net_trains_no_nan():
    ...
    opt = AdaPNM(net.parameters(), lr=1e-3)
...
def test_invalid_args_rejected():
    ...
    with pytest.raises(ValueError):
        AdaPNM(p, lr=-1.0)
    with pytest.raises(ValueError):
        AdaPNM(p, betas=(1.0, 0.999))
    with pytest.raises(ValueError):
        AdaPNM(p, beta0=1.5)
    with pytest.raises(ValueError):
        AdaPNM(p, eps=-1e-8)
    with pytest.raises(ValueError):
        AdaPNM(p, weight_decay=-0.1)
    with pytest.raises(ValueError):
        AdaPNM(p, momentum_dtype="fp8")
    with pytest.raises(ValueError):
        AdaPNM(p, bf16_method="bogus")


def test_sparse_grad_rejected():
    p = torch.nn.Parameter(torch.randn(4))
    opt = AdaPNM([p], lr=1e-3)
