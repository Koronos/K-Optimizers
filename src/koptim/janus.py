"""Janus — AdaPNM (Adam + Positive-Negative Momentum) on koptim's memory backend.

Janus (provisional code name) marries **AdaPNM** — the adaptive variant of
**Positive-Negative Momentum** (Xie et al. 2021, *Positive-Negative Momentum:
Manipulating Stochastic Gradient Noise to Improve Generalization*, ICML 2021,
arXiv:2103.17182) — to the precision and memory machinery already proven in
:class:`~koptim.adafusion.Adafusion`. It is the **generalization-bucket**
code-named experiment: an *implicit regularizer* that improves flat-minima /
train-val-gap behaviour **without** the extra forward-backward of SAM. Keeping it
a separate class lets it be A/B'd against Adafusion cleanly (Adafusion is left
...
the whole memory story. Janus therefore **defaults ``ams_bound=False``** and, when
...
semantics Adafusion/Liofusion use, applied here to the **final** ``delta`` (the
...
for consistency with the rest of koptim and because the rescale preserves step
size, but **this is the knob to ablate first** when measuring Janus's
...
second-moment helpers (:mod:`koptim._factored`), the momentum **storage layout**
and quant/dequant primitives in :mod:`koptim._momentum_codec` (int8 per-row
absmax; 4-bit per-block absmax, nibble-packed), the stochastic-rounding bf16
weight update (:func:`koptim._stochastic_rounding.add_stochastic_`),
...
and update a *single* buffer, so they cannot be reused verbatim; Janus uses the
...
class Janus(Optimizer):
    ...
    Args:
        ...
        beta0: the **positive-negative momentum coefficient** (kozistr's ``beta3``).
            ...
            ``[0, 1]``. ``beta0=0`` collapses to plain (debiased) Adam-momentum;
            ``beta0=1`` (default) is the canonical PNM ``(2*m_pos - m_neg)/sqrt(5)``.
        ...
        momentum_dtype: storage for **both** momentum buffers — ``"bfloat16"``
            ...
            Janus carries *two* momenta, so its momentum floor is ~2x a
    ...
    def _alloc_momentum(
        ...
        Storage layout matches :mod:`koptim._momentum_codec` exactly (per-row int8
    ...
    def _dequant_stacked(
        ...
        """Stacked fp32 momentum ``[N, *shape]`` from per-param storage (see Liofusion)."""
        n = len(states)
        per = 1
        for d in shape:
            per *= d
    ...
    def step(self, closure: Any = None) -> Any:
        ...
        for group in self.param_groups:
            ...
            for p in params:
                if p.grad.is_sparse:
                    raise RuntimeError("Janus does not support sparse gradients")
            group["step"] += 1
            if not params:
                continue
